...
/**
 * Login único do app — Google, via Supabase Auth (ver useAuth.ts). Não
 * tem mais um segundo modo "entrar como administrador": quem é admin é
 * decidido depois do login pelo e-mail (ver useAdminAuth.ts) e ganha um
 * botão "Área Admin" que só aparece pra ele — não existe login separado.
 *
 * `onLogin` dispara o redirect pro Google; a volta (com sessão já ativa)
 * acontece fora do ciclo de vida deste modal, então ele não espera
 * resposta — só chama e deixa a navegação acontecer.
 */
